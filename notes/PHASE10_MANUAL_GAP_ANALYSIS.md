...
> **2026-04-26** (업데이트: 2026-05-01 — Phase 11/12/13 완료 결과 반영) | fw4_0_1_2229 + MiniFreak V 4.0.2.6369 vs **양쪽 공식 매뉴얼 v4.0.1**
...
|| LFO Triggering 8 모드 | ✅ **완료** | CM4 enum @ `0x081B0E7C` — Free/PolyKbd/MonoKbd/LegatoKb/One/LFO/CycEnv/SeqStart 8종 전부 확인 (`PHASE11_GAP_FILL_ANALYSIS.md` §2) |
|| LFO Shaper 라이브러리 (16F+8U+2P) | ⚠️ **부분 완료** | Shaper 파라미터 + 25개 프리셋 (16 builtin + 8 user + Preset) 확인 (`PHASE12_RELIABILITY_UPGRADE.md`). factory 메모리 레이아웃 미확정 |
|| Cycling Envelope 모드 (Env/Run/Loop) + Stage Order | ✅ **완료** | CM4 Stage Order 3종 (RHF/RFH/HRF) 확인 (`PHASE11_GAP_FILL_ANALYSIS.md` §5). Env mode=4-stage ADSD, Run/Loop=3-stage RHF (§3.2 참조) |
...
|| **Sequencer 64-step 구조** (Step Rec/Real-time Rec/Overdub, Hold/Tie, Page nav) | ✅ **완료** | Phase 8 갭 §2.5.1. `FUN_08029390` state machine (6 cases)은 Phase 9에서 발견. Step buffer 레이아웃 확정: 24 field/step 구조 (`PHASE12_SEQ_BUFFER_LAYOUT.md`) |
...
|| **Spice/Dice 랜덤 시퀀스 생성기** (5 영역: Velocity/Octave/Gate/StepOnOff/EnvDecay+Release) | ⚠️ **부분 완료** | Phase 8 갭 §2.5.4. 확률 LUT은 발견(Phase 9), Spice/Dice quantity 펌웨어 확정 (Phase 13: 1~100 범위, `PHASE13_SPICE_DICE_QUANTITY.md`). 실제 마스크+변형 로직 미식별 |
...
> **종합**: Phase 8 갭의 **Critical/Medium은 100% 처리**, **High는 ~70% 처리** (Phase 11/12 완료분 반영 — Sequencer 64-step ✅, Spice/Dice quantity ✅).
...
#### 3.8.1 펌웨어 공통 enum 21 entry (`PHASE8_COMPLETE_REPORT.md`)

```
 0: Noise            7: Multi Filter (AP)     14: Cloud Grains     ← V3
 1: Bass             8: Surgeon Filter (AP)   15: Hit Grains       ← V3
 2: SawX             9: Comb Filter (AP)       16: Frozen           ← V3
 3: Harm            10: Phaser Filter (AP)     17: Skan             ← V3
 4: FM / RM         11: Destroy                18: Particle         ← V3
                    12: Dummy (내부용)
                    13: Audio In
                    14: Wavetable              ← V3
                    15: Sample                 ← V3
...
> **주의**: Phase 8 COMPLETE_REPORT의 인덱스 체계와 §3.1의 표기가 다름. 위는 COMPLETE_REPORT 원본 인덱스.

#### 3.8.2 Osc1 (24종) vs Osc2 (21종) 대응표

| 공통 enum idx | 펌웨어 명칭 | Osc1 | Osc2 | HW 매뉴얼 명칭 |
|:---:|---|:---:|:---:|---|
...
| 9 | Destroy | ❌ | ✅ | Osc 2 Audio Processor |
| 10 | Dummy | ❌ | ❌ | (내부 예약) |
| 11 | Audio In | ✅ | ❌ | Audio In |
| 12 | Wavetable | ✅ | ✅ | Wavetable ← V3 |
| 13 | Sample | ✅ | ✅ | Sample ← V3 |
| 14 | Cloud Grains | ✅ | ✅ | Cloud ← V3 그래뉼러 |
| 15 | Hit Grains | ✅ | ✅ | Hit ← V3 그래뉼러 |
| 16 | Frozen | ✅ | ✅ | Frozen ← V3 그래뉼러 |
| 17 | Skan | ✅ | ✅ | Skan ← V3 그래뉼러 |
| 18 | Particle | ✅ | ✅ | Particle ← V3 그래뉼러 |
| 19 | Lick | ✅ | ✅ | Lick ← V3 그래뉼러 |
| 20 | Raster | ✅ | ✅ | Raster ← V3 그래뉼러 |

**계산**: Osc1 = 21공통 - 5(Audio Processor) + 0(Audio In 포함) + 3(?) = **24종**
- Osc1에서 Audio Processor(type 5~9, 5종) 제외 → 16종 + Audio In(11) + Wavetable(12) + Sample(13) + 그래뉼러(14~20, 7종) = **24종** ✅
- Osc2 = 21공통 - 1(Dummy) - 1(Audio In) + 2(Chord/Unison 별도?) = **21종** ✅
- Osc2 전용: Chord Engine (enum 외 별도 처리), Osc1 전용: Audio In

**판정**: enum 인덱스는 Osc1과 Osc2에서 **동일** (shared index). 단 Osc2는 Audio Processor(type 5~9)를 사용하며, Osc1은 Audio In(type 11)을 사용. Chord Engine은 Osc2 전용이나 enum에 별도 항목 없음 (Osc2 처리 경로 내부에서 분기).
...
- [x] **10-3-1** `FUN_08166810` 분기 테이블 정확한 추출 (현재 `phase8_cc_param_lookup.json`은 부분 추출) → **Phase 12 완료** (`PHASE12_CC_FULL_MAPPING.md`)
- [x] **10-3-2** 각 CC case의 `vtable[3](obj, eSynthParams_index)` 호출 시 param index 추출 → **Phase 12 완료** (161 CC × 145 param 매트릭스)
...
- [x] **10-3-3** NRPN handler와 교차 검증 → **Phase 12 완료** (NRPN 33 case + Mod Matrix NRPN 22 sub-param)
- [ ] 산출물: `PHASE10_CC_FULL_MAPPING.md` (161 CC × 145 eSynthParams 매트릭스)

#### Phase 10-4: 미완료 High 항목 보완 (Phase 8 이월 + §1.5 누락분)
- [x] **10-4-1** LFO Triggering 8 모드 분기 함수 식별 → **Phase 11 완료** (`PHASE11_GAP_FILL_ANALYSIS.md` §2)
  - `LFO1 Retrig` enum (Free/Poly Kbd/Mono Kbd/Legato Kb/One/LFO/CycEnv/Seq Start) 의 8 case 분기
- [x] **10-4-2** Cycling Envelope Mode (Env/Run/Loop) + Stage Order (RHF/RFH/HRF) 분기 → **Phase 11 완료** (`PHASE11_GAP_FILL_ANALYSIS.md` §5)
...
- [x] **10-4-6** Sequencer 64-step buffer 레이아웃 (§1.5 누락분) → **Phase 12 완료** (`PHASE12_SEQ_BUFFER_LAYOUT.md`)
  - `FUN_08029390` (6 cases) state machine + step structure: 24 field/step (StepState + Gate + 6×(Pitch,Length,Velo) + 4×Mod + ModState)
...
| `PHASE10_CC_FULL_MAPPING.md` | ~~신규~~ → Phase 12 산출물로 대체 — 161 CC × 145 param 매트릭스 | 🔴 → ✅ 완료 |
...
### Phase 10-3: CC 매핑 ✅ 완료 (Phase 12)
- [x] CC#86~186 (101개) 의 90% 이상 파라미터 매핑 → **Phase 12 완료** (161 CC × 145 param, `PHASE12_CC_FULL_MAPPING.md`)
- [x] NRPN 33 case + Mod Matrix NRPN 22 sub-param 교차 검증 → **Phase 12 완료**

### Phase 10-4: 미완료 High 항목 (부분 완료)
- [x] LFO Triggering 8 모드 분기 함수 식별 → **Phase 11 완료** (`PHASE11_GAP_FILL_ANALYSIS.md` §2)
- [x] Cycling Envelope Env/Run/Loop 분기 + Stage Order 분기 식별 → **Phase 11 완료** (`PHASE11_GAP_FILL_ANALYSIS.md` §5)
- [ ] Audio In Line/Mic 게인 분기 식별
- [ ] Cutoff 실제 동작 범위 (DAC LUT 검증)
- [ ] Scale 9종 LUT + Chord 11 interval + ModQuant 10종 LUT
- [x] Sequencer 64-step buffer 레이아웃 → **Phase 12 완료** (`PHASE12_SEQ_BUFFER_LAYOUT.md`)
- [ ] MIDI Routing 4 Configuration 분기
- [ ] Velocity Curve LUT

### Phase 10-1/10-2: HW 필요 항목
...
### Phase 10-5/10-6: V 전용 / Low 우선순위
- [ ] `.mnfxmidi` 포맷 1차 디코드
- [x] Osc Type enum 21 entry ↔ Osc1 24종 ↔ Osc2 21종 대응표 완성 → **본 문서 §3.8.2에서 완료**
- [ ] 매뉴얼 충돌 7항 (Cutoff 범위, Macro 개수, Cycling Env stage, OSC type 카운트, Audio In 파라미터, All Pass, **Osc Type enum 명칭**) 모두 명확화 — §3.1~3.8에서 분석 완료, Cutoff 범위만 펌웨어 검증 대기
...
## 10. Phase 10 이후 진행 상황 (Phase 11/12/13 요약)

Phase 10 계획 수립 이후, 정적 분석 위주로 Phase 11~13이 순차 실행됨. Phase 10의 미완료 항목 중 **정적 분석 가능한 항목은 대부분 완료**됨.

### Phase 11: CM4 바이너리 직접 스캔 갭 보완 ✅
- 일치도 86.4% → **91.8%** 상향
- **Phase 10-4-1 (LFO Triggering 8모드)** ✅ — CM4 enum @ `0x081B0E7C` 8종 전부 확인
- **Phase 10-4-2 (Cycling Env Stage Order)** ✅ — CM4에서 RHF/RFH/HRF 3종 확인
- FX 12타입(CM4)/13타입(VST) enum, Voice Mode 5종, Mod Source 9종 확정
- 산출물: `PHASE11_GAP_FILL_ANALYSIS.md`

### Phase 12: 정적 분석 완료 + 매뉴얼 정정 권고 ✅
- 일치도 91.8% → **~96%** 상향
- **Phase 10-3 (CC#86~186 매핑)** ✅ — 161 CC × 145 param 정밀 매핑 (`PHASE12_CC_FULL_MAPPING.md`)
- **Phase 10-4-6 (Seq buffer)** ✅ — 24 field/step 구조 확정 (`PHASE12_SEQ_BUFFER_LAYOUT.md`)
- FX 코어 12×7 SP 매핑 (`PHASE12_FX_CORE_DSP.md`), Mod Dest ~247개 (`PHASE12_MOD_DEST_FULL.md`)
- 매뉴얼 정정 7항 + 보강 12항 (`MANUAL_CORRECTION_RECOMMENDATIONS.md`)
- 신뢰도 ★★★★★ 격상 4항목 (Vibrato, Para Env, Multi Filter, Poly2Mono)
- 산출물: `PHASE12_GAP_ANALYSIS.md`, `PHASE12_RELIABILITY_UPGRADE.md`

### Phase 13: V 매뉴얼 통합 재검증 ✅
- 3원 교차검증 (HW 매뉴얼 × V 매뉴얼 × 펌웨어), 일치도 **95.7%**
- Spice/Dice quantity 펌웨어 확정 (`PHASE13_SPICE_DICE_QUANTITY.md`)
- Poly/Dual location 분석 (`PHASE13_POLY_DUAL_LOCATION.md`)
- 산출물: `PHASE13_V_INTEGRATED_VERIFICATION.md`

### Phase 10 잔여 항목 현황

| 항목 | 상태 | 비고 |
|------|------|------|
| Phase 10-1 Collage Protocol | ❌ | HW 필요 (USB 캡처) |
| Phase 10-2 V Macro 3/4 → HW | ❌ | HW/V 필요 |
| Phase 10-3 CC 매핑 | ✅ | Phase 12 완료 |
| Phase 10-4 LFO/CycEnv/Seq | ✅ | Phase 11/12 완료 |
| Phase 10-4 Audio In/Cutoff/Scale/Chord/MIDI Routing/Velocity | ❌ | runtime 분석 또는 HW 필요 |
| Phase 10-5 V file formats | ❌ | V 플러그인 필요 |
| Phase 10-6 Low 항목들 | ❌ | 낮은 우선순위 |

> **결론**: Phase 10 계획의 **정적 분석 가능 항목은 Phase 11/12에서 모두 완료**. 잔여 항목은 모두 **HW 실기** 또는 **V 플러그인**이 필요한 runtime 분석 영역임.
