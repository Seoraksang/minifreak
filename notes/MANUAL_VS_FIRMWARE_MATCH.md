...
| 아르페지에이터 (Arp) | 5% | 95% | 93% | **95%** | 4.75 | Phase 17: Walk LUT 재판정(코드), Ratcheter/OctRandomizer 클래스 확보, 확률 알고리즘적 구현 확정 |
| 스텝 시퀀서 (Seq) | 5% | 97% | 97% | **97%** | 4.85 | — |
| 오디오 스펙/튜닝 (Audio) | 5% | 100% | 100% | **100%** | 5.00 | — |
| Spice/Dice | 3% | 88% | 85% | **88%** | 2.64 | Phase 17: Phase 13 LUT 식별 3건 전부 오류 확인, 매뉴얼 불일치 근거 철회 |
...
| | **100%** | | | | **95.89%** | |
| **종합** | **100%** | **96.0%** | **95.7%** | **95.9%** | **95.89%** | Phase 17: Walk LUT 재판정 + Spice/Dice LUT 식별 오류 3건 정정 → Arp 93→95%, S/Dice 83→88% |
...
| **Ratcheter 클래스** | ★★★★★ | CM4 RTTI `0x081AA168`: 독립 클래스로 구현 (알고리즘적 확률 계산) |
| **OctRandomizer 클래스** | ★★★★★ | CM4 RTTI `0x081AA174`: Rand Oct 독립 구현 |
| **SeqArpAdapter 클래스** | ★★★★★ | CM4 RTTI `0x081AA184`: Arp-Seq 브릿지 |
| Walk/Mutate 확률 | ★★★★☆ | 확률 LUT 미존재 — 알고리즘 계산 (float32/int 확률 상수 클러스터 없음). 매뉴얼 기술과 모순되는 증거 없음 |
| Arp 모드 인덱스 순서 | ★★★★★ | 펌웨어 Up/Down/UpDown/Rand/Walk/Pattern/Order/Poly vs 매뉴얼 순서 상이 (CORR-03) |

**Phase 16→17 상향 사유**:
1. **Walk LUT 재판정**: Phase 13에서 Walk LUT으로 식별한 `0x080546C4`는 실제로 Thumb-2 코드 (CBZ R0,+2; MOVS R0,#1)이며 데이터가 아님 → 기존 "불일치" 판정 철회
2. **확률 LUT 부재 확인**: CM4/CM7 양쪽에서 Walk(25/25/50%), Mutate(75/5/5/5/5/3/2%), RandOct(75/15/7/3%) 확률 값을 float32, uint8, uint16, Q15, Q31 모든 포맷으로 검색 → 정확한 패턴으로 저장된 LUT 없음
3. **알고리즘적 구현 확정**: Ratcheter, OctRandomizer 독립 클래스 존재 → 확률은 런타임 계산. 매뉴얼 기술값과 모순되는 펌웨어 증거 없음 → 매뉴얼 기술을 신뢰 가능
...
| 항목 | 증거 수준 | 상세 |
|------|----------|------|
| ~~spice_exp_lut~~ @ CM7 | ❌ 식별 오류 | `0x08067FDC` — Phase 13에서 Spice LUT으로 추정했으나, 실제로 narrow-range uint16 오디오 데이터 (14503~17632, 비단조). Spice/Dice와 무관 |
| ~~env_time_scale~~ @ CM7 | ✅ Tempo Table | `0x0806D330` — Phase 13에서 Spice LUT으로 추정했으나, 실제로 **20-entry float32 tempo subdivision multiplier** (dotted/triplet/straight × 7옥타브 + 2tail). Env time scale 기능 (Phase 17 확인) |
| ~~Walk LUT~~ @ CM7 | ❌ 식별 오류 | `0x080546C4` — Phase 13에서 Walk 확률 LUT으로 추정했으나, 실제로 **Thumb-2 코드** (CBZ R0; MOVS R0,#1). 데이터가 아님 |
| **Spice/Dice 문자열** | ★★★★★ | CM4 `0x081AEC10`: "Spice", "Dice" — UI 표시 문자열 확보 |
| **Spice/Dice 대상 파라미터** | ★★★★☆ | 매뉴얼 5개 파라미터 (Velocity, Octave, Gate, Step, Env) — 논리적 합리 |
| **알고리즘적 확률 구현** | ★★★★☆ | ARP 수식어와 동일 패턴: Ratcheter/OctRandomizer 독립 클래스. Spice/Dice도 LUT 미존재, 런타임 계산 추정 |

**Phase 16→17 상향 사유**:
1. Phase 13에서 Spice/Dice 점수 하향의 원인이던 3개 "LUT" 식별이 **전부 오류**였음:
   - `0x080546C4` = Thumb-2 코드 (Walk LUT 아님)
   - `0x08067FDC` = 오디오 데이터 (spice_exp_lut 아님)
   - `0x0806D330` = Tempo subdivision 테이블 (env_time_scale 맞지만 Spice/Dice 아님)
2. 기존 "LUT 불일치 → 매뉴얼 의심" 논리가 근거 없었음 → 철회
3. env_time_scale은 실제로 **엔벨로프 타임 스케일링** 전용 테이블로 확인 (20-entry, dotted/triplet/straight 패턴) → Env 섹션으로 이동 적절
...
| ENH-09 | Arp | 모호한 설명 | **확률 분포 (알고리즘적 구현 확정, 매뉴얼 기술 신뢰)** | ★★★★★ |
...
| Walk/Mutate 확률 정확값 | Arp | ~1% | USB 캡처 동적 검증 (값 자체는 매뉴얼 기술 신뢰 가능) |
| Spice/Dice 확률 분포 | S/Dice | ~3% | USB 캡처 + Dice 난수 알고리즘 추적 |
...
1. **Phase 17-2 (동적 검증)**: USB 캡처로 Walk/Mutate/Spice 확률 정확값 검증 → Arp 95%→98%, S/Dice 88%→93% 가능
2. **Phase 18 (CM7 디스어셈블리)**: OSC vtable 1:1 매핑 시도, DSP 함수 이름 복원
